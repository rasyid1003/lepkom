...
dbDriver := "mysql"
dbName := "farisrasyid_50421483"
dbUser := "root"
dbPass := ""
db, err := sql.Open(dbDriver,
dbUser+":"+dbPass+"@tcp(localhost:8083)/"+dbName)
if err != nil {
panic(err.Error())
}
return db
...
update, err := db.Prepare("UPDATE mahasiswa SET npm=?, nama=?, kelas=? WHEREid=?")
if err != nil {
log.Print(err.Error())
}
...
log.Print("Server berjalan di http://localhost:8083")
srv.ListenAndServe()
}